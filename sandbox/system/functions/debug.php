...
function debug($option=null, $state='off') {
  global $smarty;
...
function debug_lite_stack($variable){
  print('<pre>');
...
function debug_complete_stack($variables){
  print('<pre>');
  debug_print_backtrace();
...
function debug_default_stack($var, $bool=true){
  if(!$bool){
    print('<pre>');
    if(is_array($var)){
      pr($var);
...
function debug_print_array($array=array()){
  print("<pre>");
  if (is_array($array)){
