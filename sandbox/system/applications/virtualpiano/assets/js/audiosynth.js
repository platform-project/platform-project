...
! function() {
    ...
    setPub('generate', function(sound, note, octave, duration) {
        ...
        if (typeof(this._fileCache[sound][octave - 1][note][time]) != 'undefined') {
            ...
            data = data.join('');
            var chunk1 = ['fmt ', pack(1, 16), pack(0, 1), pack(0, channels), pack(1, sampleRate), pack(1, sampleRate * channels * bitsPerSample / 8), pack(0, channels * bitsPerSample / 8), pack(0, bitsPerSample)].join('');
            var chunk2 = ['data', pack(1, data.length * channels * bitsPerSample / 8), data].join('');
            var header = ['RIFF', pack(1, 4 + (8 + chunk1.length) + (8 + chunk2.length)), 'WAVE'].join('');
