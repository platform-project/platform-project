<?php

/**
 *  test
 * Test case:   This is the Platform API file
 *
 * @version     $Id: index.php {REVISIION} 2014-03-28_11h55m05s biyi $
 * @package     Platform
 * @category    Tests
 * @author      Platform
 * @link        mailto:platform@entilda.com
 * @copyright   Copyright (C) 2011 - 2014 The Platform Authors. All rights reserved.
 * @license     GNU Public Licence, see LICENSE.md
 * Platform is free software. This version may have been modified pursuant
 * to the GNU General Public License, and as distributed it includes or
 * is derivative of works licensed under the GNU General Public License or
 * other free or open source software licenses.
 * See COPYRIGHT.md for copyright notices and details.
 */

// initilizing platform for self-contained objects
platform_launch_initialize();

function check_updates()
{
    ob_start();
    @header('Content-Type: application/json');
    $repositoryPath = '/var/www/platform';

    // Change directory to the Git repository
    chdir($repositoryPath);

    // Execute Git commands to fetch updates and check status
    exec('git fetch', $fetchOutput, $fetchReturn);
    exec('git status', $statusOutput, $statusReturn);

    // Check for errors in fetching
    if ($fetchReturn !== 0) {
        http_response_code(500);
        echo json_encode(['hasUpdate' => false]);
        exit();
    }

    // Check if the status output indicates that there are new updates
    $hasUpdate = false;
    foreach ($statusOutput as $line) {
        if (strpos($line, 'Your branch is behind') !== false) {
            $hasUpdate = true;
            break;
        }
    }

    // Send JSON response
    echo json_encode(['hasUpdate' => $hasUpdate]);
    return ob_get_clean();
}

//echo check_updates();
