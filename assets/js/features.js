$(document).ready(function () {
    ...
    function renderFeatures() {
        ...
        features.forEach(feature => {
            ...
            const featureDiv = $(`<div class="feature ${feature.installed ? 'feature-installed' : 'feature-not-installed'}" data-id="${feature.id}">
                ...
                <div>
                    <a class="toggle-feature button" href="${feature.link}">Open</a> <br />
                    <button class="toggle-feature">${feature.installed ? 'Uninstall' : 'Install'}</button>
                </div>
            </div>`);
    ...
    function addFeature(name, description, link) {
        const id = 'feature' + (features.length + 1);
        features.push({ id, name, description, link,  installed: false });
        saveFeatures();
        renderFeatures();
    }
    ...
    $('#add-feature').click(function () {
        ...
        const link = prompt("Enter feature link:");
        if (name && description && link) {
            addFeature(name, description, link);
        }
    });
